 ...
 * LightboxNK.js
 * Version: 0.0.1
 * Based on : Lightbox 0.0.6 By Victor Diego <victordieggo@gmail.com>
 * Added : Support for local video, corrected the non-minified version (constructor didn't work), added a very quick and dirty wrapper to launch the lightbox starting process after other js callback
 * ================================================================== */

/*qnd wrapper to launch lightbox drill after our js stuff*/
function startLighBox(){
  ...
  const lightbox = function(element){
    ...
    const sortContent = (content) => {
     ...
     if (content.match(/\.(mov|mp4|mpeg|avi)/)) {
        const video = [];
        var extension = content.match(/\.(mov|mp4|mpeg|avi)/)[0] ;
        extension.replace(".", "") ;
        ...
        video.player = doc.createElement('video');
        video.player.setAttribute('controls', '');
        video.player.setAttribute('autoplay', 'true');
        video.player.className = 'lightbox-video-player';
        //video.player.src = 'https://' + video.url + video.id + video.options;
        video.source = doc.createElement('source');
        video.source.src = video.url ;
        video.source.type = "video/"+extension ;
      ...
      if (content.match(/\.(mov|mp4|mpeg|avi)/)) {
        ...
        video.player.type = "video/".extension ;
